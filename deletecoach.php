...
<br>
delete coach: <?php echo $_POST["deleteCoach"]; ?> <br>


     <?php
     
     
     if(!($stmt = $mysqli->prepare("DELETE FROM COACH WHERE coach_id = ?")))
     {
     echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
     }

     //Bind the parameters. S, I, and I for name, age, and team_val3. Use a POST array
     if(!($stmt->bind_param("i",$_POST['deleteCoach'])))
     {
     ...
     {
     echo "deleted " . $stmt->affected_rows . " rows (coach) from COACH.";
     }
